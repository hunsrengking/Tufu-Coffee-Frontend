  ...
  return (
    <header className={`fixed top-0 right-0 z-40 bg-white border-b border-slate-200 px-6 lg:px-8 h-20 flex items-center justify-between shadow-sm transition-all duration-300 ${isSidebarOpen ? 'left-64' : 'left-0'}`}>
      <div className="flex items-center gap-4 flex-1">
        ...
        {/* Search Input */}
        <div className="relative hidden max-w-md flex-1 lg:flex items-center">
      ...
      <div className="flex items-center gap-4 ml-auto">
        {/* Dark Mode Toggle */}
        <button className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-50 border border-slate-200 text-slate-500 hover:text-slate-800">
        ...
        <div className="relative">
          ...
          {isProfileOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-md shadow-lg py-1 z-50">
              <a href="#profile" className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2"><i className="fa-regular fa-user w-4 text-center"></i> Your Profile</a>
              <a href="#settings" className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2"><i className="fa-solid fa-gear w-4 text-center"></i> Settings</a>
              <div className="border-t border-slate-100 my-1"></div>
              <a href="#logout" className="block px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2"><i className="fa-solid fa-right-from-bracket w-4 text-center"></i> Sign out</a>
            </div>
          )}
        </div>
      </div>
